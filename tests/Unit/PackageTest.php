...
it('exposes name and alias', function () {
    $package = new Package(
        name: 'pestphp/pest',
        version: '3.8.1',
...
it('matches on raw name and alias', function () {
...
it('reports major version', function () {
    $package = new Package('vue', '3.4.0', PackageSource::NPM);

    expect($package->major())->toBe(3);
});

it('defaults to zero major when version is empty', function () {
